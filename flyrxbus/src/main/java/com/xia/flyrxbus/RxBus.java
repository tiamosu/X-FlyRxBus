...
public final class RxBus {
    ...
    private <T> void subscribe(final Object subscriber,
        ...
        if (isSticky) {
            ...
            if (stickyEvent != null) {
                ...
                final Disposable stickyDisposable = FlowableUtils.subscribe(stickyFlowable, onNext, mOnError);
                CacheUtils.getInstance().addDisposable(subscriber, stickyDisposable);
            } else {
                Utils.logW("sticky event is empty.");
            }
        }
        final Disposable disposable = FlowableUtils.subscribe(
    ...
    private <T> Flowable<T> toFlowable(final Class<T> eventType,
                                       final String tag,
                                       final Scheduler scheduler) {
        final Flowable<T> flowable = mBus.ofType(TagMessage.class)
    ...
    public abstract static class Callback<T> {
        public abstract void onEvent(String tag, T t);
    }
}